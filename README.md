...
- 即時模式：透過同源輕量代理讀取新北市 YouBike Open Data，每五分鐘與來源更新節奏對齊；資料變動後以同站、同時段歷史基線產生 30／60 分鐘啟發式風險指標，沒有唯一歷史對照時只顯示即時庫存。
...
歷史回放情境只會在使用者切換到「歷史回放」或開啟告警／調度／站點詳情頁時按需載入。首頁即時模式只取得 manifest 與當下 30／60 分鐘所需的精簡基線檔，不預取完整回放資料。新北市來源 API 沒有開放任意網站直接瀏覽器呼叫，因此保留一個只轉送資料、不解析 JSON 的 Pages Function。
