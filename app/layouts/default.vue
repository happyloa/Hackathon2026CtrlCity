...
const themeToggle = computed(() =>
  theme.value === "dark"
    ? { label: "淺色模式", ariaLabel: "切換為淺色主題", icon: "solar:sun-2-outline" }
    : { label: "深色模式", ariaLabel: "切換為深色主題", icon: "solar:moon-outline" },
);
      ...
      <header class="border-b border-line bg-canvas">
        ...
        >
          ...
          <div class="flex flex-wrap items-center gap-2">
            ...
            <button
              type="button"
              class="inline-flex min-h-11 items-center gap-2 rounded-lg border border-line bg-surface px-3 text-base font-semibold text-muted transition-colors hover:border-line-strong hover:text-ink"
              :aria-label="themeToggle.ariaLabel"
              :title="themeToggle.ariaLabel"
              @click="toggleTheme"
            >
              <Icon class="text-xl text-accent" :icon="themeToggle.icon" />
              {{ themeToggle.label }}
            </button>
          </div>
        </div>
