...
<template>
  <section class="warning-panel"
    :class="{ 'has-warnings': hasWarnings, 'forecast-unavailable': !hasForecast && !hasWarnings }" aria-label="缺車通報">
    <button class="warning-trigger" type="button" :aria-expanded="expanded" :aria-controls="warningContentId"
      @click="expanded = !expanded">
      <span class="warning-symbol">
        <component :is="!hasForecast && !hasWarnings ? Info : hasWarnings ? BellRing : CheckCircle2"
          style="width: 1em; height: 1em" :stroke-width="2" aria-hidden="true" />
      </span>
    ...
    <div v-if="expanded" :id="warningContentId" class="warning-content">
      ...
      <div class="flex flex-wrap gap-2 px-4 pb-3 pt-1" role="group" aria-label="缺車通報分類">
        <button v-for="tab in TABS" :key="tab.key" type="button" :aria-pressed="activeTab === tab.key"
          ...
          {{ tab.label }}（{{ tab.kind === 'forecast' && !forecastAvailable[tab.key as HorizonKey] ? '—' :
            countsByTab[tab.key] }}）
        </button>
      </div>

      <div v-if="activeTabDef.kind === 'forecast'">
        <div v-if="forecastByHorizon[activeTabDef.key as HorizonKey].length"
          ...
          <button v-for="item in forecastByHorizon[activeTabDef.key as HorizonKey]" :key="item.station.id" type="button"
            ...
            <span
              class="ml-auto flex w-full items-center justify-between gap-2 text-body1 sm:w-auto sm:flex-col sm:items-end">
              <span class="text-muted">預估可借車 <strong class="font-mono text-ink">{{ item.predictedBikesDisplay
                  }}</strong> 台</span>
        ...
        <div v-else
          ...
          <component class="icon-md" :is="forecastAvailable[activeTabDef.key as HorizonKey] ? CheckCircle2 : Info"
            style="width: 1em; height: 1em" :stroke-width="2" aria-hidden="true" />{{ forecastAvailable[activeTabDef.key as HorizonKey] ? '這個時點已對照的站點，沒有預測缺車情形。' : '這個時點沒有可用預測，請先查看即時站況。' }}
        </div>
      </div>
      <div v-else>
        <div v-if="realtimeByTab[activeTabDef.key as RealtimeTabKey].length"
          class="divide-y divide-line border-t border-line">
          <button v-for="station in realtimeByTab[activeTabDef.key as RealtimeTabKey]" :key="station.id" type="button"
            ...
            <span
              class="ml-auto flex w-full items-center justify-between gap-2 text-body1 sm:w-auto sm:flex-col sm:items-end">
              <span class="text-muted">目前可借車 <strong class="font-mono text-ink">{{ station.availableBikes }}</strong>
                台</span>
...
.warning-panel.forecast-unavailable {
  border-left-color: var(--muted);
}

.forecast-unavailable .warning-symbol {
  color: var(--muted);
...
.has-warnings .warning-figures strong {
  color: var(--warning);
}
