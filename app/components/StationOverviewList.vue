  ...
  <section class="panel overflow-hidden" aria-labelledby="station-list-title">
    <header class="flex flex-col gap-2 border-b border-line p-4 sm:flex-row sm:items-center sm:justify-between">
      <div>
        ...
        <h2 id="station-list-title" class="mt-1 text-h6 font-bold">{{ stations.length }} 個站點</h2>
      </div>
      <span class="text-body1 font-semibold text-muted">需要注意的站點排在前面</span>
    </header>
    ...
    <div v-if="visibleStations.length" class="divide-y divide-line">
      <article v-for="station in visibleStations" :key="station.id" class="grid grid-cols-[2.5rem_minmax(0,1fr)] items-center gap-3 border-l-4 px-4 py-3 sm:grid-cols-[2.5rem_minmax(0,1fr)_auto]" :class="rowClasses(station)" :style="rowStyle(station)">
        <span class="grid size-10 shrink-0 place-items-center rounded-full border bg-panel text-h6" :class="statusClasses(station)">
        ...
        <button type="button" class="min-w-0 flex-1 text-left hover:underline" @click="emit('select', station.id)">
          <strong class="block break-words text-body1 text-ink">{{ displayStationName(station.name) }}</strong>
          <span class="mt-0.5 block text-body1 text-muted">{{ station.district }} · 可借 {{ station.availableBikes }} · 可還 {{ station.availableDocks }}</span>
        </button>
        <div class="col-start-2 flex flex-wrap items-center gap-2 sm:col-start-3 sm:row-start-1 sm:justify-end">
          <span class="rounded-md border bg-panel px-2 py-1 text-body1 font-bold" :class="statusClasses(station)">{{ statusLabel(station) }}</span>
          <strong v-if="statusFor(station) === 'empty' || statusFor(station) === 'full'" class="w-14 text-right font-mono text-body1 text-ink">{{ Math.round(alertLookup.get(station.id)?.priorityScore || 0) }} 分</strong>
        </div>
      </article>
    </div>
    <div v-else class="flex min-h-32 items-center justify-center gap-2 p-4 text-center text-body1 font-semibold text-muted">
      <Icon class="icon-md" icon="solar:magnifer-outline" />找不到符合條件的站點
    </div>

    <div v-if="remainingCount" class="border-t border-line p-3 text-center">
      <button type="button" class="min-h-11 rounded-md border border-line-strong bg-panel px-4 text-body1 font-bold text-accent hover:bg-panel-muted" @click="visibleCount += pageSize">再顯示 {{ Math.min(pageSize, remainingCount) }} 個</button>
    </div>
  </section>
</template>
