...
import { ArrowUpRight, Check, ChevronDown, CircleHelp, ListFilter, Search, Settings, TriangleAlert } from '@lucide/vue'
...
const props = withDefaults(defineProps<{ stations: StationRisk[]; alerts: Alert[]; pageSize?: number; resetKey?: string }>(), { pageSize: 12 })
    ...
    <section v-for="(group, index) in groups" :key="group.district" class="station-district-group">
      ...
      <div v-if="isOpen(group.district, index)" :id="`station-district-${group.district}`">
        ...
        <article v-for="station in group.stations.slice(0, visible(group.district))" :key="station.id" class="station-row" :data-status="statusFor(station)" :style="rowStyle(station)">
          <button class="station-row-name" type="button" @click="emit('select', station.id)"><component :is="statusIcon(station)" :size="17" aria-hidden="true" /><strong>{{ displayStationName(station.name) }}</strong></button>
...
<style scoped>
.station-groups { display: grid; gap: 14px; }.station-list-heading { display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 8px; }.station-list-heading h2 { display: flex; align-items: center; gap: 7px; font-size: 17px; font-weight: 650; }.station-list-heading h2 svg { color: var(--accent); }.station-list-heading p, .station-list-heading > span { color: var(--muted); font-size: 12px; }.station-list-heading p { margin-top: 3px; }
.station-district-group { background: var(--panel); border: 1px solid var(--line); border-radius: 9px; overflow: hidden; }.station-district-header { display: flex; align-items: center; gap: 16px; padding-right: 16px; }.station-district-toggle { display: flex; align-items: center; gap: 10px; min-width: 0; flex: 1; padding: 15px 16px; min-height: 62px; text-align: left; }.station-district-toggle > svg { flex-shrink: 0; color: var(--muted); }.station-district-toggle > svg.collapsed { transform: rotate(-90deg); }.station-district-toggle > strong { font-size: 17px; font-weight: 650; white-space: nowrap; }.station-district-toggle > span { font-size: 12px; color: var(--muted); }.station-district-toggle > small { margin-left: auto; color: var(--muted); font-size: 11px; }.station-group-counts { display: flex; gap: 12px; margin-left: 10px; }.station-group-counts b { font-weight: 500; }.station-group-counts [data-status="empty"] { color: var(--danger); }.station-group-counts [data-status="full"] { color: var(--info); }.station-district-dispatch { display: flex; align-items: center; justify-content: center; gap: 4px; min-height: 38px; padding: 6px 10px; border: 1px solid var(--line-strong); border-radius: 6px; font-size: 12px; color: var(--ink); text-decoration: none; white-space: nowrap; }.station-district-dispatch:hover { border-color: var(--accent); color: var(--accent); }
.station-table-heading, .station-row { display: grid; grid-template-columns: minmax(200px, 1fr) 110px 160px 85px 80px; align-items: center; gap: 12px; padding: 10px 20px; }.station-table-heading { font-size: 11px; color: var(--muted); background: color-mix(in srgb, var(--panel-muted) 40%, var(--panel)); border-top: 1px solid var(--line); }.station-table-heading > span:nth-child(n+3) { text-align: right; }.station-table-heading > span:last-child { text-align: center; }
.station-row { --status-color: var(--muted); --priority-width: 0%; min-height: 66px; border-top: 1px solid var(--line); border-left: 3px solid transparent; background: linear-gradient(to right, color-mix(in srgb, var(--status-color) 8%, transparent) 0%, color-mix(in srgb, var(--status-color) 8%, transparent) var(--priority-width), transparent var(--priority-width)); }.station-row[data-status="empty"] { --status-color: var(--danger); border-left-color: var(--danger); }.station-row[data-status="full"] { --status-color: var(--info); border-left-color: var(--info); }.station-row[data-status="unknown"] { --status-color: var(--warning); }.station-row[data-status="stable"] { --status-color: var(--positive); }.station-row-name { display: flex; align-items: center; gap: 10px; min-height: 44px; text-align: left; min-width: 0; }.station-row-name > svg { flex-shrink: 0; color: var(--status-color); }.station-row-name strong { font-size: 14px; font-weight: 550; overflow-wrap: anywhere; }.station-row-name:hover strong { text-decoration: underline; text-underline-offset: 3px; }.station-row-status { color: var(--status-color); font-size: 12px; }.station-row-inventory { display: flex; justify-content: end; align-items: baseline; gap: 10px; font-size: 14px; font-variant-numeric: tabular-nums; }.station-row-inventory b { font-weight: 550; }.station-row-inventory small { font-size: 11px; color: var(--muted); }.station-row-inventory i { font-style: normal; color: var(--muted); }.station-row-score { text-align: right; font-variant-numeric: tabular-nums; }.station-row-score b { display: inline-block; min-width: 55px; text-align: center; padding: 3px 6px; border: 1px solid color-mix(in srgb, var(--status-color) 40%, var(--line)); border-radius: 5px; color: var(--status-color); font-size: 16px; font-weight: 600; }.station-row-score small { font-size: 11px; font-weight: 400; }.station-row-score > span { color: var(--muted); }.station-row-detail { display: flex; align-items: center; justify-content: center; gap: 5px; min-height: 40px; border: 1px solid var(--line-strong); border-radius: 6px; font-size: 12px; }.station-row-detail:hover { border-color: var(--accent); color: var(--accent); }.station-group-more { display: flex; align-items: center; justify-content: space-between; gap: 10px; padding: 8px 20px; border-top: 1px solid var(--line); color: var(--muted); font-size: 11px; }.station-group-more button { min-height: 40px; display: flex; align-items: center; gap: 6px; color: var(--accent); font-size: 12px; }
.station-list-empty { display: grid; place-items: center; gap: 10px; padding: 44px 20px; border: 1px solid var(--line); border-radius: 9px; background: var(--panel); color: var(--muted); text-align: center; }.station-list-empty strong { color: var(--ink); font-size: 16px; }.station-list-empty > span { font-size: 13px; }
...
@media (max-width: 740px) { .station-list-heading > span { display: none; }.station-district-header { padding-right: 10px; gap: 4px; }.station-district-toggle { flex-wrap: wrap; column-gap: 7px; row-gap: 2px; padding: 12px; }.station-district-toggle strong { font-size: 16px; }.station-district-toggle > small { display: none; }.station-group-counts { flex-basis: 100%; padding-left: 26px; margin-left: 0; }.station-table-heading { display: none; }.station-row { grid-template-columns: minmax(0,1fr) auto auto; gap: 7px 10px; padding: 12px; }.station-row-name { grid-column: 1 / 4; min-height: 32px; }.station-row-status { grid-column: 1; grid-row: 2; }.station-row-inventory { grid-column: 1; grid-row: 3; justify-content: start; }.station-row-score { grid-column: 2; grid-row: 2 / 4; }.station-row-detail { grid-column: 3; grid-row: 2 / 4; padding: 4px 9px; }.station-row-name strong { font-size: 15px; }.station-group-more { padding: 8px 12px; } }
</style>
