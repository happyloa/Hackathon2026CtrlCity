...
const groups = computed(() => {
  ...
  return [...result].map(([district, stations]) => ({ district, stations: [...stations].sort((a, b) => compareStationValues(sortValue(a), sortValue(b), sortDirection.value) || displayStationName(a.name).localeCompare(displayStationName(b.name), 'zh-Hant') || a.id.localeCompare(b.id)),
...
<template>
  <section class="station-groups" aria-labelledby="station-list-title">
    ...
    <div class="station-sort-controls">
      <label>區內排序 <select v-model="sortKey"><option v-for="option in sortOptions" :key="option.value" :value="option.value">{{ option.label }}</option></select></label>
      <button type="button" @click="sortDirection = sortDirection === 'desc' ? 'asc' : 'desc'">{{ sortDirection === 'desc' ? '由高到低 ↓' : '由低到高 ↑' }}</button>
      <span>維持時間每 6 分鐘加 1 分，最多加 10 分；總分上限 100。</span>
    </div>
    ...
    <section v-for="(group, index) in groups" :key="group.district" class="station-district-group">
      ...
      <div v-if="isOpen(group.district, index)" :id="`station-district-${group.district}`">
        ...
        <article v-for="station in group.stations.slice(0, visible(group.district))" :key="station.id" class="station-row" :data-status="statusFor(station)" :style="rowStyle(station)">
          ...
          <span class="station-row-duration" title="依連續快照估算無車／無位至今的分鐘數；快照每 5 分鐘記錄，資料不足或非目前無車／無位時顯示 —"><small>狀態維持 </small>{{ durationLabel(station) }}</span>
...
<style scoped>
.station-sort-controls { display: flex; align-items: center; flex-wrap: wrap; gap: 10px; color: var(--muted); font-size: var(--type-body2); }
.station-sort-controls select, .station-sort-controls button { min-height: 40px; padding: 6px 10px; border: 1px solid var(--line-strong); border-radius: 6px; background: var(--panel); color: var(--ink); }
.station-table-heading button { min-height: 32px; text-align: inherit; }
.station-table-heading button:hover { color: var(--accent); }
...
@media (max-width: 1000px) { .station-table-heading, .station-row { grid-template-columns: minmax(160px, 1fr) 90px 140px 190px 80px 65px; gap: 8px; padding-right: 12px; padding-left: 12px; }.station-row-inventory { gap: 6px; } }
@media (max-width: 900px) { .station-list-heading > span { display: none; }.station-district-header { padding-right: 10px; gap: 4px; }.station-district-toggle { flex-wrap: wrap; column-gap: 7px; row-gap: 2px; padding: 12px; }.station-district-toggle strong { font-size: var(--type-body1); }.station-district-toggle > small { display: none; }.station-group-counts { flex-basis: 100%; padding-left: 26px; margin-left: 0; }.station-table-heading { display: none; }.station-row { grid-template-columns: minmax(0,1fr) auto auto; gap: 7px 10px; padding: 12px; }.station-row-name { grid-column: 1 / 4; min-height: 32px; }.station-row-duration { grid-column: 1 / 4; grid-row: 4; text-align: left; }.station-row-duration small { display: inline; font-size: inherit; }.station-row-status { grid-column: 1; grid-row: 2; }.station-row-inventory { grid-column: 1; grid-row: 3; justify-content: start; }.station-row-score { grid-column: 2; grid-row: 2 / 4; }.station-row-detail { grid-column: 3; grid-row: 2 / 4; padding: 4px 9px; }.station-row-name strong { font-size: var(--type-body1); }.station-group-more { padding: 8px 12px; } }
</style>
