export const protectedPublishPaths = ['_headers', '_routes.json', '_nuxt/*', 'raw/*', 'snapshots/*', 'state/*', 'data/operational-adjustments.json', 'data/demand-events.json', 'data/manual-routes.json']
...
export function frontendEnvironment(outputs) {
  ...
  return {
    ...
    NUXT_PUBLIC_MANUAL_ROUTES_ENDPOINT: login ? '/api/v1/manual-routes' : '',
    ...
    NUXT_PUBLIC_ADJUSTMENTS_ENDPOINT: login ? '/api/v1/adjustments' : '',
    NUXT_PUBLIC_EVENTS_ENDPOINT: login ? '/api/v1/events' : '',
