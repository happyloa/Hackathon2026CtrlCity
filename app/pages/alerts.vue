  ...
  <div class="subpage">
    <section class="subpage-hero panel">
      <div>
        ...
        <h2>先確認風險，再安排人員處置。</h2>
        <p>{{ mode === 'live' ? '告警由最新官方庫存與 60 分鐘歷史基線即時計算。' : '告警來自所選歷史回放時點，可用來重現值班判斷。' }}確認狀態只存在於目前 Demo 瀏覽器工作階段。</p>
      </div>
    ...
    <div v-if="activePending && !dashboard" class="loading-board"><Icon icon="svg-spinners:3-dots-fade" />{{ mode === 'live' ? '正在取得官方即時資料…' : '正在載入歷史調度情境…' }}</div>
