  ...
  <div class="mx-auto w-full max-w-screen-2xl space-y-4 px-4 py-4 sm:px-6 lg:px-8 lg:py-6">
    ...
    <section v-else-if="record" class="panel flex flex-col gap-4 p-4 sm:p-5 lg:flex-row lg:items-center lg:justify-between">
      <div class="min-w-0">
        ...
        <h2 class="mt-2 text-h5 font-bold tracking-tight">{{ stationName }}</h2>
        <p class="mt-2 max-w-3xl text-body1 leading-7 text-muted">{{ stationDescription }}</p>
      </div>
      <div class="grid shrink-0 grid-cols-2 gap-2">
        <span class="flex min-h-16 flex-col justify-center rounded-lg bg-surface px-3 text-body1 font-semibold text-muted">可借車 <b class="mt-1 text-h6 text-ink">{{ record.station.availableBikes }}</b></span>
        <span class="flex min-h-16 flex-col justify-center rounded-lg bg-surface px-3 text-body1 font-semibold text-muted">可還位 <b class="mt-1 text-h6 text-ink">{{ record.station.availableDocks }}</b></span>
      </div>
    </section>
    <div v-if="record" class="grid grid-cols-1 gap-4 xl:grid-cols-2">
      <section v-if="mode === 'historical_replay'" class="panel overflow-hidden">
        <div class="border-b border-line px-4 py-4 sm:px-5"><p class="section-kicker">歷史庫存走勢</p><h2 class="mt-2 text-h6 font-bold tracking-tight">最近 6 小時＋基線推估</h2></div>
      ...
      <section class="panel overflow-hidden">
        <div class="border-b border-line px-4 py-4 sm:px-5"><p class="section-kicker">{{ mode === 'live' ? '即時預估' : '歷史預估' }}</p><h2 class="mt-2 text-h6 font-bold tracking-tight">{{ mode === 'live' ? '60 分鐘站況' : '風險細節' }}</h2></div>
        <template v-if="canShowPrediction">
          <div class="mx-4 mt-4 flex flex-wrap items-end gap-x-3 gap-y-1 rounded-lg bg-warning-surface p-4 text-warning sm:mx-5"><strong class="text-h3 leading-none">{{ Math.round((forecast?.riskScore || 0) * 100) }}</strong><span class="text-body1 font-semibold leading-6">風險排序分／100</span></div>
          <dl class="grid grid-cols-2 gap-3 px-4 py-4 sm:px-5"><div class="rounded-lg border border-line p-3"><dt class="text-body1 font-semibold text-muted">預估可借車</dt><dd class="mt-1 text-h5 font-bold">{{ forecast?.predictedBikes }}</dd></div><div class="rounded-lg border border-line p-3"><dt class="text-body1 font-semibold text-muted">預估可還位</dt><dd class="mt-1 text-h5 font-bold">{{ forecast?.predictedDocks }}</dd></div></dl>
        </template>
        <template v-else>
          <div class="mx-4 mt-4 flex flex-col gap-1 rounded-lg bg-surface p-4 sm:mx-5"><span class="text-body1 font-semibold"><b>{{ unavailableForecastTitle }}</b></span><span class="text-body1 leading-6 text-muted">{{ unavailableForecastExplanation }}</span></div>
          <dl class="grid grid-cols-2 gap-3 px-4 py-4 sm:px-5"><div class="rounded-lg border border-line p-3"><dt class="text-body1 font-semibold text-muted">目前可借車</dt><dd class="mt-1 text-h5 font-bold">{{ record.station.availableBikes }}</dd></div><div class="rounded-lg border border-line p-3"><dt class="text-body1 font-semibold text-muted">目前可還位</dt><dd class="mt-1 text-h5 font-bold">{{ record.station.availableDocks }}</dd></div></dl>
        </template>
        <ul class="space-y-2 border-t border-line px-4 py-4 sm:px-5"><li v-for="reason in forecast?.reasons" :key="reason" class="flex items-start gap-2 text-body1 leading-6 text-muted"><Icon class="mt-1 shrink-0 icon-md text-accent" icon="solar:check-read-outline" /> {{ reason }}</li></ul>
      </section>
    </div>
