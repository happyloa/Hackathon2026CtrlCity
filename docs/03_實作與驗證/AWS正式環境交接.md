...
- 已提供環境檢查、部署、發布、知識上傳與 smoke test 指令。
...
不要把 access key、secret、session token 或 ARN 放進 Git、前端環境變數或截圖。
...
`aws:publish` 會讀取 stack output、建置並上傳前端。Agent 未啟用時，不會顯示分析入口。
...
這一步只上傳 Markdown，不建立向量庫或啟動 ingestion。確認 region、embedding model 與主辦方流程後，再接 Managed Knowledge Base／Gateway。

## 成本界線

- Agent、知識 bucket 與付費 smoke test 都預設關閉；只有加上 `--include-agent --confirm-agent-cost` 才會呼叫一次 Agent。
