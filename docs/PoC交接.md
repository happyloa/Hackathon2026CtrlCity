...
### 下一步：3 個可選方向（尚未決定，待討論）

Precision 全站改善幅度不大（+2.4pp）是目前最大的疑問，以下是討論過的候選方向，尚未選定：

1. **加鄰站空間特徵**（PoC 九步計畫原訂 Step 8）：也許上下游鄰站的即時狀態能補上額外訊號，提升 precision。
2. **先接受現有結果，直接往下做**：全站 F1 有改善即可視為「驗證過 ML 有優勢」，且已證實市區（站數多）改善明顯，先把其他 PoC 項目（前端整合、demo）做完；小站區可以規則 baseline 跟模型並用（依站數切換）。
3. **調參或加特徵再評估一次**：目前只用了當前庫存+四組滯後+動量，還沒加入 `station-time-profile` 的訓練期統計特徵（歷史同站同時段空/滿率），這是最可能大幅提升 precision 的方向；也可以考慮針對小站區改用不同策略（例如站數低於某個門檻時直接 fallback 回規則 baseline，而不是硬套模型）。
